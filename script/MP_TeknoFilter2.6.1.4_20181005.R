...
#                                 Version 2.6.1.4. Updated 2018-09-25 by MP
############################################################################################################
...
vTAGFILENAME = cbind(TagFilename=c("tag_lists/ucd/2018FriantTagList_gps20190115.csv"),PRI=c(5)) # tag list file

DoCleanPrePre = FALSE     # real time files cleaning command
DoCleanRT = FALSE         # real time files cleaning command
DoCleanShoreRT = FALSE    # real time files cleaning command

DoCleanJST = FALSE        # tekno file cleaning command
DoCleanSUM = FALSE        # ats-style tekno file cleaning command
DoCleanLotek = TRUE       # lotek file cleaning command
DoCleanATS = FALSE        # ats file cleaning command
...
# filter requires a separate sub-directory for each receiver file type
RAW_DATA_DIR = "data/"
TEKNO_SUBDIR = "Tekno/" # or "" if not in a subdirectory of data directory
ATS_SUBDIR = "ATS/" # or ""
LOTEK_SUBDIR = "Lotek/cleaned_w_UCD_tags/" # "Lotek/cleaned with UCD tags/" # or ""
...
# Lines 60-82 are functions that Matt wrote to switch the direction of the slashes when copying and pasting
# a windows directory path into R.  THEY ARE NOT NECESSARY FOR THE CODE TO RUN (pers comm. MP 6/10/2019)
  ...
  if (length(u)>0) {
    ...
    for(i in u){
      ...
      if (ans2[,.N]>0) {
        ...
        setkey(dat,dtf)
        setkey(keep,x)
        res <- rbind(res, dat[keep][Hex==i]) #added '[Hex==i]'
      }
      iter <- iter+1
    } 
    close(timerbar)
